...
public class ProductRestController {
	...
	public void updateProduct(@ModelAttribute Product3 product, Model model, Principal principal) {
	...
	// 상품업데이트 처리
	@RequestMapping(value = "/updateEndProduct", method = RequestMethod.POST)
	public void updateEndProduct(@ModelAttribute Product3 product, Model model, Principal principal) {
		User user = userService.readUser(principal.getName());
		product.setProvidernumber(user.getProvidernumber());
		product.setUsername(user.getUsername());
		product.setAuditId(user.getUsername());
		productService.updateEndProduct(product);
		return;
	}
	...
	public void updateProductPackage(@ModelAttribute ProductPackage productpackage, Model model, Principal principal) {
	...
	public void saveProductPackage(@ModelAttribute ProductPackage productpackage, Model model, Principal principal){
		...
		if ("NEW".equals(productpackage.getCompositionProductId())) {
			Product3 product = productService.findProductid();
			product.setProductType("NORMAL");
			product.setPackageYn("N");
			product.setProductName(productpackage.getCompositionproductName());
			product.setSubscribeStartDateTime(productpackage.getEffectStartDateTime());
			product.setSubscribeEndDateTime(productpackage.getEffectEndDateTime());
			product.setPriceAmount(productpackage.getPriceamount());
			product.setPackagePriceReferenceYn("");
			product.setPackagePriceBandwidth("");
			product.setPackageVarietyYn("");
			product.setProductDescription("");
			product.setSuspendPriceAmount(0);
			product.setProvidernumber(user.getProvidernumber());
			product.setAuditId(user.getUsername());
			productService.saveProduct(product);
			productpackage.setCompositionProductId(product.getProductId());
		}
	...
	public void saveDiscountUpdate(@ModelAttribute Discount discount, Model model, Principal principal) {

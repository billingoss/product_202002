...
public class PaymentController {
	...
	public ModelAndView viewMobileReturn(Model model, HttpServletRequest request, HttpServletResponse response,Principal principal) {
		...
		try {
			...
			providerInformation.setDetailCode("BILLLETTER");		
			...
			if ("사용자에 의한 취소입니다.".equals(resultmsg)) {				
				//사용자 취소일 경우 주문요청 페이지로 이동
				response.sendRedirect(request.getContextPath() + "/billletter/productsale?conNumber="+conNumber);
			}else {				
	...
	public ModelAndView viewMobileMakeBillKeyReturn(Model model, HttpServletRequest request, HttpServletResponse response,Principal principal) {
		...
		try {
			...
			if ("사용자에 의한 취소입니다.".equals(resultmsg) || "[cancel] 사용자에 의한 취소입니다.".equals(resultmsg)) {				
				...
				response.sendRedirect(request.getContextPath() + "/billletter/productsaledetail?channelCustomerNumber=&providerNumber="+contInfo.getProviderNumber()+"&customerName="+customerName+"&phoneNumber="+contInfo.getCellPhoneNumber());
			}else {				
				model.addAttribute("deliveryList",deliveryList);
				model.addAttribute("contInfo", contInfo);
				//model.addAttribute("invoiceList", invoiceList);
	...
	private  Map<String, String> mobileMakeBillKey(HttpServletRequest request, Principal principal) throws Exception{
...
//				paymentHistoryInput.setPaymentMethodCode(iniPayInfo.getPayMethod());
	...
	public  Map<String, String> onlineMakeBillKey(HttpServletRequest request, Principal principal) throws Exception{
		//return object

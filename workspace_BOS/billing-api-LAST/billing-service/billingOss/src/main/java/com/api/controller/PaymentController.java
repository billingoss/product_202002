...
public class PaymentController {
	...
	//Local Test
	//private String inipayHome = "C:\\projects\\workspace\\billing-api\\billing-service\\billingOss\\src\\main\\resources\\templates\\IniPay45";	 //local Test
	...
	public  Map<String, Object> setPaymentReqInfo(@ModelAttribute IniPayInfo iniPayInput , Model model,HttpServletRequest request, Principal principal) {		
		...
		String billType = StringUtil.nullConvert(iniPayInput.getBillType()).toUpperCase();
		String channelgubun = StringUtil.nullConvert(iniPayInput.getChannelGubun()).toUpperCase();
		String jobGubun = StringUtil.nullConvert(iniPayInput.getJobGubun()).toUpperCase();
		...
		try {
			...
			*/
			...
			//가맹점 결제 정보 조회
			//Map<String, String> providerPayInfo = this.getProviderInicisInfo(contInfo.getProviderNumber());			
			Map<String, String> providerPayInfo = this.getProviderInicisInfo(contInfo.getProviderNumber(), contInfo.getConNumber(), "INIPAYINFO");		
			if (providerPayInfo == null || providerPayInfo.size() == 0) {
				throw new BizException("가맹점 결제 정보가 없습니다.");
			}			
			String mid = providerPayInfo.get("MID");		// 가맹점 ID(가맹점 수정후 고정)
			String signKey = providerPayInfo.get("SIGNKEY");	// 가맹점에 제공된 웹 표준 사인키(가맹점 수정후 고정)
			String merchantkey = providerPayInfo.get("MERCHANTKEY");	//상점 대칭			
			...
			//String price = Integer.toString(contInfo.getPriceAmount());						// 상품가격(특수기호 제외, 가맹점에서 직접 설정)
			String price = Integer.toString(contInfo.getTotalinvoiceamount());						// 상품가격(특수기호 제외, 가맹점에서 직접 설정)
			
			//일괄 결제일 경우 일괄결제 금액을 가져 온다
			if("N".equals(contInfo.getRecurringInvoiceYn())) {
				price =Integer.toString(contInfo.getBulkAmount());
			...
			if ("MOBILE".equals(channelgubun)) {
				...
				if ("MAKEBILLKEY".equals(jobGubun)) {					
					iniPayForm.append("	<input id=\"returnurl\" name=\"returnurl\" value=\""+siteDomain+"iniPayMobileMakeBillKeyReturn\" />");
				}else {					
				...
				iniPayForm.append("	<input id=\"period\" name=\"period\" value=\""+offerPeriod+"\" />");				
			}else {
				//일반 웹결제
				String offerPeriod = contInfo.getEffectStartDateTime() +"-"+contInfo.getEffectEndDateTime();
				...
				if ("MAKEBILLKEY".equals(jobGubun)) {
					//빌키 재생성일 경우 returnUrl 분리.자동결제변경
					iniPayForm.append("	<input id=\"returnUrl\" name=\"returnUrl\" value=\""+siteDomain+"makeBillKeyReturn\" />");
				}else {					
			...
			iniPayForm.append("	<input id=\"jobGubun\"  name=\"jobGubun\" value=\""+jobGubun+"\" />");
			...
			//CARDPOINT:HPP(2):va_receipt:below1000:SKIN():KWPY_TYPE(0):KWPY_VAT(0)
			if ("HPP".equals(billType)) {
			...
			log.debug("Bill Key 생성 오류");
			throw new BizException("Bill Key 생성 오류");
	...
	public ModelAndView viewMobileReturn(Model model, HttpServletRequest request, HttpServletResponse response,Principal principal) {
		...
		//provider의 return url 
		String saleMainPage = "";

		try {
			
			//Billkey 가 중복 될 경우 잘못된 접근으로 처리
			...
			int billKeyCount = this.invoiceservice.getBillKeyCount(billKey);
			
			//같은 Billkey가 있을 경우 예외 처리
			if(billKey != null && billKeyCount > 0) {
				String orderid = resultMap.get("orderid");
				
				//계약번호
				int beginIndex = orderid.indexOf("_")+1;
				int conNumber = Integer.parseInt(orderid.substring(beginIndex));
				
				contInfo = new ContractInfoDetail();//계약정보	
				//계약정보 상세 조회
				...
				providerInformation.setProviderNumber(contInfo.getProviderNumber());
				providerInformation.setCode("SALEMAINPAGE");
				providerInformation.setDetailCode(contInfo.getChannelId().toUpperCase());		
				providerInformation = this.providerService.getProviderInformationOne(providerInformation);
				saleMainPage = providerInformation.getValue1();		
				
				model.addAttribute("saleMainPage", saleMainPage);
				
				throw new BizException("중복 요청된 정보 입니다.");
			}
			
			
			//#####################
			// BillKey 생성 결과 setting
			//#####################
			
			Map<String, Object> resultMakeBillKey = this.mobileMakeBillKey(model, request, principal);
			
			int conNumber = (int)resultMakeBillKey.get("conNumber");
			contInfo = (ContractInfoDetail)resultMakeBillKey.get("contInfo");	//계약정보	
			String resultmsg = (String)resultMakeBillKey.get("resultmsg");		//결과메시지	
			String resultcode = (String)resultMakeBillKey.get("resultcode");	//결과코드
			String billkey = (String)resultMakeBillKey.get("billkey");				//BillKey	
			saleMainPage = (String)resultMakeBillKey.get("saleMainPage");	//판매 메인 페이지
			String preFixUrl = (String)resultMakeBillKey.get("preFixUrl");	//판매 메인 페이지
			String cardcd = (String)resultMakeBillKey.get("cardcd");	//판매 메인 페이지
			String paymentMethodCode = (String)resultMakeBillKey.get("paymentMethodCode");	//판매 메인 페이지
			String cardno = (String)resultMakeBillKey.get("cardno");	//판매 메인 페이지
			String hppcorp = (String)resultMakeBillKey.get("hppcorp");	//판매 메인 페이지
			...
			if("00".equals(resultcode)){
				...
				idi.setProviderNumber(contInfo.getProviderNumber());
				...
				//기결제된 건수 조회
				int paymentCloseCount = this.invoiceservice.getInvoicePaymentCloseCount(idi);
			
								
				if (invoice != null && paymentCloseCount == 0) {					
					//invoice 정보 설정
					PaymentHistoryInput paymenthistoryinput = new PaymentHistoryInput();							 
					paymenthistoryinput.setInvoiceNumber(invoice.getInvoiceNumber());
					...
					paymenthistoryinput.setCardCorporationCode(cardcd);
					paymenthistoryinput.setProviderNumber(contInfo.getProviderNumber());							
					...
					if("CARD".equals(paymentMethodCode)){//빌링결제
						...
						iniPayInfo.setInvoiceNumber(invoice.getInvoiceNumber());
						//iniPayInfo.setPayMethod(paymentMethodCode);
						iniPayInfo.setAuditId(contInfo.getAuditId());
					...
					if (paymentCloseCount > 0) {						
						resultcode = "00";
						resultmsg = "정상 결제 처리 되었습니다.";
			...
			if ("사용자에 의한 취소입니다.".equals(resultmsg) || "[cancel] 사용자에 의한 취소입니다.".equals(resultmsg)) {				
				//사용자 취소일 경우 주문요청 페이지로 이동
				response.sendRedirect(request.getContextPath() +preFixUrl+ "/productsale?conNumber="+conNumber);
				return null;
			}else {				
			...
			ContractInsert contractInsert = new ContractInsert();
			contractInsert.setConNumber(contInfo.getConNumber());
			//판매완료 정보 조회
	 ...
	 * @return
	 */
	@RequestMapping(value = "/iniPayMobileMakeBillKeyReturn", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
	public ModelAndView viewMobileMakeBillKeyReturn(Model model, HttpServletRequest request, HttpServletResponse response,Principal principal) {
		log.info("===================inicis-iniPayMobileMakeBillKeyReturn=====================");
		ModelAndView modelAndView = new ModelAndView();
		modelAndView.setViewName("inipay/mobileMakeBillKeyReturn");
		
		
		
		//provider의 return url 
		String saleMainPage = "";
		ContractInfoDetail contInfo = null;
		try {
			...
			String billkey = (String)resultMakeBillKey.get("billkey");				//BillKey	
			saleMainPage = (String)resultMakeBillKey.get("saleMainPage");	//판매 메인 페이지
			String preFixUrl = (String)resultMakeBillKey.get("preFixUrl");		//url
			String cardcd = (String)resultMakeBillKey.get("cardcd");				//카드구분코드
			String paymentMethodCode = (String)resultMakeBillKey.get("paymentMethodCode");	 //결제방법코드
			...
			if ("사용자에 의한 취소입니다.".equals(resultmsg) || "[cancel] 사용자에 의한 취소입니다.".equals(resultmsg)) {				
				...
				String customerName = URLEncoder.encode(contInfo.getCustomerName(),"UTF-8");
				response.sendRedirect(request.getContextPath() + preFixUrl+"/productsaledetail?channelCustomerNumber=&providerNumber="+contInfo.getProviderNumber()+"&customerName="+customerName+"&phoneNumber="+contInfo.getCellPhoneNumber());
				return null;
			}else {				
				model.addAttribute("deliveryList",deliveryList);
				model.addAttribute("contInfo", contInfo);
			...
			// TODO: handle exception
			model.addAttribute("resultCode", "BILL_01");
			model.addAttribute("resultMsg", "카드정보 수정 요청 실패!["+e+"]");
			log.error("==>"+e);
			//페이지 이동
		}finally {
			ContractInsert contractInsert = new ContractInsert();
	 ...
	 * @return
	 */
	//@RequestMapping(value = "/mobileMakeBillKey", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
	private  Map<String, Object> mobileMakeBillKey(Model model, HttpServletRequest request, Principal principal) throws Exception{
		
		Map<String, Object> resultMakeBillKey = new HashMap<String, Object>();
		...
		String cardcd = resultMap.get("cardcd");
		
		resultMakeBillKey.put("resultcode", resultcode);
		resultMakeBillKey.put("resultmsg", resultmsg);
		resultMakeBillKey.put("orderid", orderid);
		resultMakeBillKey.put("billkey", billkey);
		resultMakeBillKey.put("cardno", cardno);
		resultMakeBillKey.put("hppcorp", hppcorp);
		resultMakeBillKey.put("buyername", buyername);
		resultMakeBillKey.put("cardcd", cardcd);
		
		//계약번호
		int beginIndex = orderid.indexOf("_")+1;
		int conNumber = Integer.parseInt(orderid.substring(beginIndex));
		resultMakeBillKey.put("conNumber", conNumber);
		
		ContractInfoDetail contInfo = new ContractInfoDetail();//계약정보	
		//계약정보 상세 조회
		contInfo = this.invoiceservice.getContractInfoDetail(conNumber);	
		
		resultMakeBillKey.put("contInfo", contInfo);
		...
		providerInformation.setDetailCode(contInfo.getChannelId().toUpperCase());		
		providerInformation = this.providerService.getProviderInformationOne(providerInformation);
		String saleMainPage = providerInformation.getValue1();		
		
		resultMakeBillKey.put("saleMainPage", saleMainPage);
		
		String saleMain[] = saleMainPage.split("/");
		String preFixUrl ="/";
		
		for (int i = 0; i < saleMain.length; i++) {
			if (i >1) {
				break;
			}
			preFixUrl += saleMain[i];
		}
		resultMakeBillKey.put("preFixUrl", preFixUrl);
		...
		if("00".equals(resultcode)){	
			...
			/*************************************************************************************************
			//Billkey 변경 이력 등록 - 사용자에 의한 결제 정보 변경 History 를 저장 한다.
			**************************************************************************************************/			
			BillKeyHistory billKeyHistory = new BillKeyHistory();
			billKeyHistory.setConNumber(conNumber);
			billKeyHistory.setPaymentMethodCode(paymentMethodCode);
			billKeyHistory.setCardCorporationCode(payment.getCardCorporationCode());
			billKeyHistory.setBeforBillKey(contInfo.getBillKey());
			billKeyHistory.setCurrentBillKey(billkey);
			billKeyHistory.setAuditId(contInfo.getAuditId());
			this.invoiceservice.insertBillKeyHistory(billKeyHistory);
	...
	public Map<String, String> payProc(@ModelAttribute IniPayInfo iniPayInfoIn, String channelDiv, Principal principal) throws Exception{
		...
		IniPayInfo iniPayInfo = this.invoiceservice.getIniPayInfo(iniPayInfoIn);
		if (iniPayInfo != null) {
			iniPayInfo.setMemo(iniPayInfoIn.getMemo());
		}
		
		//결제 금액이 없을 경우 예외 처리
		if(iniPayInfo.getPrice() <= 0) {
			throw new BizException("결제 대상 금액이 없습니다.");			
		}
		
		/*ID mapping*/		
		if ("MOBILE".equals(channelDiv) || "BATCH".equals(channelDiv) || principal == null) {
			//모바일일 경우 Login 정보 없음
			providernumber = iniPayInfoIn.getProviderNumber();
			userName = iniPayInfoIn.getAuditId();
		}else {			
		...
		//가맹점 결제 정보 조회
		//Map<String, String> providerPayInfo = this.getProviderInicisInfo(iniPayInfoIn.getProviderNumber());		
		Map<String, String> providerPayInfo = this.getProviderInicisInfo(iniPayInfoIn.getProviderNumber(), iniPayInfoIn.getConNumber(), "INIPAYINFO");
		if (providerPayInfo == null || providerPayInfo.size() == 0) {
			throw new BizException("가맹점 결제 정보가 없습니다.");
		}
		
		String mid = providerPayInfo.get("MID");		// 가맹점 ID(가맹점 수정후 고정)
		String keyPW = providerPayInfo.get("KEYPW");
		...
		//홈페이지 주소(URL) 설정
		iniPayInfo.setUrl("https://www.s-billing.com");
		...
		//################
		  ...
		  data.setData("price", Integer.toString(iniPayInfo.getPrice()));					// 가격
		...
		{			
			paymentHistoryInput.setErrorReasonCode("000");
			// DB 처리
			int resultCnt = billingController.savePaymentInfo(paymentHistoryInput);
			...
			//TODO 납부 이력에 오류 코드와 오류 내역을 저장 한다.
			paymentHistoryInput.setErrorReasonCode("999");
			paymentHistoryInput.setPgErrorCode(resultCode);
			resultMsg = "결제실패"+resultMsg;
			paymentHistoryInput.setPgErrorMsg(resultMsg);
			paymentHistoryInput.setPaymentMethodCode(iniPayInfo.getPayMethod());
			paymentHistoryInput.setPaymentTypeCode("PAYMENTERROR");
	...
	public  Map<String, Object> payCancleProc(@ModelAttribute PaymentHistoryInput paymentHistoryInput) throws Exception{		
		...
		/*ID mapping*/
		//User user = userService.readUser(principal.getName());
		int providernumber = paymentHistoryInput.getProviderNumber();
		...
		if (resultCnt > 0) {
			throw new BizException("배송상태를 확인 하세요.");
		}
		...
		//# 2. 환불 처리 DB 저장 #
		//################
			String memo = paymentHistoryInput.getMemo();
			paymentHistoryInput = this.invoiceservice.getPaymentHistoryInfo(paymentHistoryInput);
			if (paymentHistoryInput == null) {
				throw new BizException("환불 대상 결제내역이 없습니다.");
			}
			paymentHistoryInput.setUserName(userName);
			paymentHistoryInput.setMemo(memo);
			
			//가맹점 결제 정보 조회
			//Map<String, String> providerPayInfo = this.getProviderInicisInfo(paymentHistoryInput.getProviderNumber());		
			Map<String, String> providerPayInfo = this.getProviderInicisInfo(paymentHistoryInput.getProviderNumber(), paymentHistoryInput.getConNumber(), "INIPAYINFO");
			if (providerPayInfo == null || providerPayInfo.size() == 0) {
				throw new BizException("가맹점 결제 정보가 없습니다.");
			}
		...
		//################
			...
			data.setData("cancelMsg", paymentHistoryInput.getMemo());				// INIpay User ID
		...
		//################
			...
			if (!"00".equals(resultCode)) {				
				//DB rollback
				resultMsg = "취소실패"+resultMsg;
				paymentHistoryInput.setErrorReasonCode("999");
				paymentHistoryInput.setPgErrorCode(resultCode);
				paymentHistoryInput.setPgErrorMsg(resultMsg);
//				paymentHistoryInput.setPaymentMethodCode(iniPayInfo.getPayMethod());
				paymentHistoryInput.setPaymentTypeCode("REFUNDERROR");	
				invoiceservice.setPaymentHistory(paymentHistoryInput);
				//throw new BizException("환불 처리에 실패 했습니다.\n"+resultMsg);
			}else {
				//환불 이력을 저장 한다.
				resultCnt = this.invoiceservice.setRefundHistory(paymentHistoryInput);
		 		
				//결제 금액 복원
				resultCnt = this.invoiceservice.updaterefundColInvoice(paymentHistoryInput);
				//결제 금액 상세 복원
				resultCnt = this.invoiceservice.updaterefundColInvoiceDetail(paymentHistoryInput);
				
				//배송상태 변경
				//배송 정보의 상태 값을 배송대기(BEFORE) 상태에서 계약완료(ORDERDONE) 상태로 변경 한다.
				paymentHistoryInput.setDeliveryState("ORDERDONE");
				
				//선불일 경우 배송 상태를 변경 하지 않는
				resultCnt = this.invoiceservice.updateDeliverydetailByDeliverystate(paymentHistoryInput);
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value = "/makeBillKeyReturn", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
	public ModelAndView makeBillKeyReturn(Model model, HttpServletRequest request, HttpServletResponse response,Principal principal) throws Exception{
		log.info("===================inicis-return=====================");
		ModelAndView modelAndView = new ModelAndView();
		modelAndView.setViewName("inipay/webMakeBillKeyReturn");
		/*ID mapping*/
		User user = userService.readUser(principal.getName());
		int providernumber = user.getProviderNumber();
		String userName = user.getUsername();
		...
		Map<String, String> billKeyResult = this.onlineMakeBillKey(request, principal); 
		model.addAttribute("billKeyResult", billKeyResult);		
		return modelAndView;
	}
	...
	private  Map<String, String> onlineMakeBillKey(HttpServletRequest request, Principal principal) throws Exception{
		//return object
		...
		try{
			...
			**********************************/
				       ...
				       ***************************************************************************************************/	
						...
						if (!secureSignature.equals(resultMap.get("authSignature")) && "0000".equals(resultMap.get("resultCode"))) {						
							...
							if ("0000".equals(resultMap.get("resultCode"))) {
								throw new BizException("데이터 위변조 체크 실패");
							}
						}
					}
			...
			log.debug("BillKey 생성 오류");
			throw new BizException("BillKey 생성 오류");
		}
		return resultMakeBillKey;
	}	
	
	/**
	 * Provider 별 이니시스 가맹점 정보를 가져 온다.
	 * @param providerNumber
	 * @return
	 */
	private Map<String, String> getProviderInicisInfo(int providerNumber){
		Map<String, String> iniPayInfo = new HashMap<>();
		
		ProviderInformation providerInput = new ProviderInformation();
		providerInput.setProviderNumber(providerNumber);
		providerInput.setCode("INIPAYINFO");
		
		List<ProviderInformation> providerInfo = this.providerService.getProviderInformationByCode(providerInput);
		
		for (ProviderInformation providerInformation : providerInfo) {
			iniPayInfo.put(providerInformation.getDetailCode(), providerInformation.getValue1());
		}
	 ...
	 * @return
	 */
	private Map<String, String> getProviderInicisInfo(int providerNumber, int conNumber, String pgGroup){
		Map<String, String> iniPayInfo = new HashMap<>();
		
		//계약건이 어느 채널인지 조회 한다.
		ContractInfoDetail contDetail= this.invoiceservice.getContractInfoDetail(conNumber);
		
		//채널/업체 별 Pg 정보를 조회하기 위하여 파라메터를 설정 한다.
		ChannelPGInformation pgInfo = new ChannelPGInformation();
		pgInfo.setChannelId(contDetail.getChannelId());
		pgInfo.setProviderNumber(providerNumber);
		pgInfo.setPgGroup(pgGroup);
		
		//채널/업체 별 Pg 정보를 조회
		List<ChannelPGInformation> pgInforList = this.channelService.selectChannelPGInformationList(pgInfo);
		
		
		//조회된 PG 정보를 Key, value로 담는다.
		for (ChannelPGInformation channelPGInformation : pgInforList) {
			iniPayInfo.put(channelPGInformation.getPgKey(), channelPGInformation.getPgValue());			
		}
